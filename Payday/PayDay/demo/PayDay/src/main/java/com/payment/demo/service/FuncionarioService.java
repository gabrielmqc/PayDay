...
public class FuncionarioService {
    ...
    public Funcionario encontrarFuncionarioPorNIS(String nis) {
        try {
            ...
            for (Funcionario funcionario : funcionarios) {
                ...
                funcionario.setSalarioBruto(0f);
                ...
                funcionario.setInss(0f);
                funcionario.setFgts(0f);
                funcionario.setIrrf(0f);
                // zera outros campo
    ...
    public void executarEventos(Funcionario funcionario) {
        ...
        if (funcionario.getEventos().getFaltas() != null) {
            eventosService.subtrairFaltas(funcionario, funcionario.getNivelSalarial(),
                    funcionario.getEventos().getFaltas());
        }
        if (funcionario.getEventos().getAtraso() != null) {
            eventosService.descontoPorAtraso(funcionario, funcionario.getNivelSalarial(),
                    funcionario.getEventos().getAtraso());
        }
        ...
        if (funcionario.getEventos().getDiariaViagem() != null) {
            eventosService.diariaViagem(funcionario, funcionario.getNivelSalarial(),
                    funcionario.getEventos().getDiariaViagem());
        }
    ...
    public static float calcularINSS( float salarioBruto) {
        ...
        if (salarioBase <= 1100) {
            inss = salarioBase * 0.075f;
            
        } else if (salarioBase <= 2203.48) {
            inss = salarioBase * 0.09f;
        } else if (salarioBase <= 3305.22) {
            inss = salarioBase * 0.12f;
        } else if (salarioBase <= 6433.57) {
            inss = salarioBase * 0.14f;
        } else {
            inss = 6433.57f * 0.14f;
    ...
    public static float calcularIRRF(float salarioBruto) {
        ...
        float salarioBase = salario - inss;
        float irrf;
